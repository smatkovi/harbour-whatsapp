...
This app was written with the help of an LLM (large language model) —
specifically Claude, made by Anthropic. In other words, a lot of the
code was generated by "AI" (a term I put in quotation marks on purpose,
since it is really an LLM, not intelligence in any meaningful sense).

What the LLM did *not* do: it did not decide what to build, it did not
test anything on real hardware, and it did not judge whether the result
was actually good. Those parts are mine. The architecture decisions, the
feature choices, the on-device testing before every release, and the
responsibility for what ships all rest with me — the LLM was a coding
assistant, nothing more.

I mention this openly because I would rather be upfront about it than
have someone "discover" it later. Issues and patches are welcome
regardless of how any particular line came to exist.
